...
library ArrayUtils {
    ...
    function unsafeWriteAddress(uint256 index, address source)
        ...
        returns (uint256)
    {
        uint256 conv = uint256(uint160(address(source))) << 0x60;
        assembly {
            mstore(index, conv)
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    function staticCall(
        ...
        bytes memory combined = new bytes(calldata_.length + extradata.length);
        uint256 index;
        assembly {
            index := add(combined, 0x20)
        }
        index = ArrayUtils.unsafeWriteBytes(index, extradata);
        ArrayUtils.unsafeWriteBytes(index, calldata_);
        assembly {
            result := staticcall(
                gas(),
                target,
        ...
        returns (bytes32 hash)
    {
        ...
        index = ArrayUtils.unsafeWriteBytes(index, order.calldata_);
        ...
        returns (bytes32)
    {
        ...
        require(validateOrder(hash, order, sig), "requireValidOrder");
        return hash;
    }
    ...
    function validateOrder(
        ...
        /* or (b) ECDSA-signed by maker. */
        if (btc_ecrecover(btcu_message_hash(hash), sig.v, sig.r, sig.s) == order.maker) {
            return true;
        }
    ...
    /**
     * @dev Return whether or not two orders can be matched with each other by basic parameters (does not check order signatures / calldata_ or perform static calls)
     * @param buy Buy-side order
     * @param sell Sell-side order
    ...
    function atomicMatch(
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "validateOrderParametersBuy");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "validateOrderParametersSell");
        } else {
            sellHash = requireValidOrder(sell, sellSig);
        }

        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "ordersCanMatch");
        ...
        /* Must match calldata_ after replacement, if specified. */
        ...
        require(ArrayUtils.arrayEq(buy.calldata_, sell.calldata_));
        ...
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata_));
...
contract Exchange is ExchangeCore {
    ...
    /**
     * @dev Return whether or not two orders' calldata_ specifications can match
     * @param buyCalldata Buy-side order calldata_
     * @param buyReplacementPattern Buy-side order calldata_ replacement mask
     * @param sellCalldata Sell-side order calldata_
     * @param sellReplacementPattern Sell-side order calldata_ replacement mask
     * @return Whether the orders' calldata_ can be matched
     */
...
contract WyvernExchangeN is Exchange {
    ...
    constructor(
        ...
        ERC20 tokenAddress,
        address payable protocolFeeAddress
    ) public {
